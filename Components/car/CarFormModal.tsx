...
export function CarFormModal({ editCar, branches: branchesProp, onClose, onSubmit }: CarFormModalProps) {
  ...
  useEffect(() => {
    if (branchesProp.length > 0) { setBranches(branchesProp); return; }
  ...
  const [registrationNumber,     setRegistrationNumber]     = useState(editCar?.registrationNumber ?? "");
  const [vinNumber,              setVinNumber]              = useState(editCar?.vinNumber ?? "");
  const [branchId,               setBranchId]               = useState(editCar?.branch?.id ?? "");
  const [currentStatus,          setCurrentStatus]          = useState(editCar?.currentStatus ?? "Active");
const [insuranceStatus, setInsuranceStatus] = useState<InsuranceStatus>(
  (editCar?.insuranceStatus as InsuranceStatus) ?? "Valid"
);  const [registrationExpiryDate, setRegistrationExpiryDate] = useState(editCar?.registrationExpiryDate?.slice(0, 10) ?? "");
  const [insuranceExpiryDate,    setInsuranceExpiryDate]    = useState(editCar?.insuranceExpiryDate?.slice(0, 10) ?? "");
  const [inspectionExpiryDate,   setInspectionExpiryDate]   = useState(editCar?.inspectionExpiryDate?.slice(0, 10) ?? "");
  const [gpsDeviceId,            setGpsDeviceId]            = useState(editCar?.gpsDeviceId ?? "");
  const [year,     setYear]     = useState<number | undefined>(editCar?.year ?? new Date().getFullYear());
  const [capacity, setCapacity] = useState<number | undefined>(editCar?.capacity ?? undefined);
  const [weight,   setWeight]   = useState<number | undefined>(editCar?.weight ?? undefined);
  ...
  const parseNum = (v: string): number | undefined => v.trim() === "" ? undefined : Number(v);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errs = validate({ manufacturer, model, year, plateNumber, plateLetters });
    if (Object.keys(errs).length) { setErrors(errs); return; }

    const raw: Record<string, unknown> = { manufacturer, model, year, plateNumber, plateLetters, currentStatus };
    ...
    if (registrationExpiryDate) raw.registrationExpiryDate = toIsoDateTime(registrationExpiryDate);
    if (insuranceExpiryDate)    raw.insuranceExpiryDate    = toIsoDateTime(insuranceExpiryDate);
    if (inspectionExpiryDate)   raw.inspectionExpiryDate   = toIsoDateTime(inspectionExpiryDate);
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <label style={labelStyle}>
              الشركة المصنعة *
              <input ref={firstRef} style={inputStyle("manufacturer")} value={manufacturer}
                onChange={e => { setManufacturer(e.target.value); clearFieldError("manufacturer"); }}
                placeholder="تويوتا" dir="rtl" autoComplete="off" />
              {errors.manufacturer && <span style={errorTextStyle}>{errors.manufacturer}</span>}
            </label>
            <label style={labelStyle}>
              الموديل *
              <input style={inputStyle("model")} value={model}
                onChange={e => { setModel(e.target.value); clearFieldError("model"); }}
                placeholder="لاند كروزر" dir="rtl" />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.75rem" }}>
            <label style={labelStyle}>
              سنة الصنع *
              <input style={inputStyle("year")} type="number" min={1900} max={new Date().getFullYear() + 1}
                value={year ?? ""}
                onChange={e => { setYear(parseNum(e.target.value)); clearFieldError("year"); }}
                dir="ltr" />
              {errors.year && <span style={errorTextStyle}>{errors.year}</span>}
            </label>
            <label style={labelStyle}>
              اللون
              <input style={inputBase} value={color} onChange={e => setColor(e.target.value)} placeholder="أبيض" dir="rtl" />
            </label>
            <label style={labelStyle}>
              نوع اللوحة
              <input style={inputBase} value={plateType} onChange={e => setPlateType(e.target.value)} placeholder="خاص" dir="rtl" />
            </label>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <label style={labelStyle}>
              رقم اللوحة *
              <input style={inputStyle("plateNumber")} value={plateNumber}
                onChange={e => { setPlateNumber(e.target.value); clearFieldError("plateNumber"); }}
                placeholder="1234" dir="ltr" />
              {errors.plateNumber && <span style={errorTextStyle}>{errors.plateNumber}</span>}
            </label>
            <label style={labelStyle}>
              حروف اللوحة *
              <input style={inputStyle("plateLetters")} value={plateLetters}
                onChange={e => { setPlateLetters(e.target.value); clearFieldError("plateLetters"); }}
                placeholder="أ ب ج" dir="rtl" />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <label style={labelStyle}>
              رقم الاستمارة
              <input style={inputBase} value={registrationNumber} onChange={e => setRegistrationNumber(e.target.value)} placeholder="SA-001234" dir="ltr" />
            </label>
            <label style={labelStyle}>
              رقم الهيكل (VIN)
              <input style={inputBase} value={vinNumber} onChange={e => setVinNumber(e.target.value)} placeholder="1HGBH41JXMN109186" dir="ltr" />
            </label>
            <label style={labelStyle}>
              انتهاء الاستمارة
              <input style={inputBase} type="date" value={registrationExpiryDate} onChange={e => setRegistrationExpiryDate(e.target.value)} />
            </label>
            <label style={labelStyle}>
              حالة التأمين
              <select style={{ ...inputBase, cursor: "pointer" }} value={insuranceStatus ?? "Valid"}
            ...
            <label style={labelStyle}>
              انتهاء التأمين
              <input style={inputBase} type="date" value={insuranceExpiryDate} onChange={e => setInsuranceExpiryDate(e.target.value)} />
            </label>
            <label style={labelStyle}>
              انتهاء الفحص الدوري
              <input style={inputBase} type="date" value={inspectionExpiryDate} onChange={e => setInspectionExpiryDate(e.target.value)} />
            </label>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.75rem" }}>
            <label style={labelStyle}>
              الفرع
              <select style={{ ...inputBase, cursor: "pointer" }} value={branchId} onChange={e => setBranchId(e.target.value)} dir="rtl">
            ...
            <label style={labelStyle}>
              الحالة
              <select style={{ ...inputBase, cursor: "pointer" }} value={currentStatus}
                onChange={e => setCurrentStatus(e.target.value as Car["currentStatus"])} dir="rtl">
            ...
            <label style={labelStyle}>
              رقم GPS
              <input style={inputBase} value={gpsDeviceId} onChange={e => setGpsDeviceId(e.target.value)} placeholder="GPS-001" dir="ltr" />
            </label>
            <label style={labelStyle}>
              الطاقة الاستيعابية
              <input style={inputBase} type="number" min={0} value={capacity ?? ""} onChange={e => setCapacity(parseNum(e.target.value))} placeholder="0" dir="ltr" />
            </label>
            <label style={labelStyle}>
              الوزن (كجم)
              <input style={inputBase} type="number" min={0} value={weight ?? ""} onChange={e => setWeight(parseNum(e.target.value))} placeholder="0" dir="ltr" />
            </label>
          </div>
